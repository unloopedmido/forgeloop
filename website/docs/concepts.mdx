...
Every scaffolded project includes a **ForgeLoop manifest**: project name, `preset`, `language`, `packageManager`, enabled features (database, tooling, logging, git, docker, CI), and the generated path layout. The manifest is written to disk as **`forgeloop.config.mjs`** and is what `forgeloop info` and `forgeloop doctor` read.
