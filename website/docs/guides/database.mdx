...
Database wiring is **imported from the entry path** (`basic` inlines `connectDatabase` in `index`; `modular`/`advanced` call it before login). Pick **modular** or **advanced** if you also want **`forgeloop add`** and **`deploy commands`**.
